...
export async function emailExists(correo = "") {
  ...
  export async function impactLevelExists(impactLevel = "") {
    if(impactLevel !== "Alto" && impactLevel !== "Medio" && impactLevel !== "Bajo"){
        throw new Error("Invalid impact level");
    }
  }

  export async function categoryExists(category = "") {
    const allowedCategories = ["IT", "Servicios Financieros", "Manufactura", "Salud", "Comercio", "Educación", "Alimentos", "Construcción", "Comunicación y entretenimiento", "Energía", "Transporte", "Turismo", "Agricultura"];
